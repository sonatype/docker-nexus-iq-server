...
#
# ---------------------------------------------------------------------------
# Image validation (goss). These stages fork off `runtime-base` and contribute nothing to
# the shipped image.
#
#   docker build --target test-results --no-cache-filter test \
#       --output type=local,dest=build/test-results .
#
# IMPORTANT: `runtime` must stay the LAST stage. With no --target BuildKit builds the last
# stage, and build_and_push_images.sh passes no --target -- an export stage last would get
# tagged and pushed.
# ---------------------------------------------------------------------------
#

# `builder` already has curl and tar, and runs on the target platform so uname -m is right.
...
# GOSS_RETRY_TIMEOUT/GOSS_SLEEP from the environment, which silently re-enabled retry.
ARG READY_TIMEOUT=300s
ARG READY_SLEEP=5s
# Forces the tests to re-run: the test RUN has no file inputs, so BuildKit would serve a
# cached pass forever. Referenced in the RUN because BuildKit only rekeys on args in use.
ARG CACHEBUST=""
# hadolint ignore=DL3066
USER root
...
#      producing concatenated XML documents no JUnit parser accepts. A readiness failure
#      is reported as a synthesized single-testcase XML instead.
...
# The shipped image. MUST be last so it is the default target. No instructions: the stage
# inherits runtime-base's layers and full image config.
